...
@Slf4j
class PredicatesHolderDateTimeClassService4buildPredicateCore4ActionEnumBefore4IntegratedTest
    ...
    protected void setUp() {
        super.setUp();
        ((FilterDateTimeApiObject) filterAdApiRequest.getCreatingTime().getValue().getValue()).setAction(FilterDateTimeApiObject.ActionEnum.BEFORE);
    }
    ...
    void base4notExistedValue4Test() {
        ...
        // Prepare
        filterAdApiRequest.getCreatingTime().getValue().getNegate().setNegate(false);
        ((FilterDateTimeApiObject) filterAdApiRequest.getCreatingTime().getValue().getValue()).setOnDate(LocalDateTime.now().plusYears(1));
